...
#we've imported the libraries ive used, needed pandas for this

#next we create 
def create_dataset(filename, rows, cols, min, max):
    random.seed(4)
    syndata = num.random.uniform(low=min, high=max, size=(rows, cols))
    frame = panda.DataFrame(syndata)
    frame.to_csv(filename, index=False)#this method essentially takes the arguments and constructs a csv file for us that is pasted in the same folder as the py file
...
def plot_silhouette(k_values, silhouette_scores):
    ...
    plot.title('plotting the ratios of silhouette and clusters for the synthetic dataset created')
    plot.grid(True)
    plot.savefig('KMeanssynthetic.png')
    plot.show()

#calling our methods from the main fucntion
def main():
    #specifying what kind of ranges we want in our csv file (kept the ranges between 1 and -1 since the dataset had similar data)
    filename = 'synthetic_data.csv'
    rows = 300
    cols = 300
    min = -1
    max = 1
    create_dataset(filename, rows, cols, min, max)
